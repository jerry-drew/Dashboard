...
const Sidebar = () => {
  ...
  // Use useEffect to handle click events outside the sidebar
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if the clicked element is not part of the sidebar
      if (activeMenu && screenSize <= 900 && !event.target.closest('.sidebar-container')) {
        setActiveMenu(false);
      }
    };
  ...
  }, [activeMenu, setActiveMenu, screenSize]);
  ...
  return (
    <div className='ml-3 h-screen md:overflow-hidden overflow-auto md:hover:overflow-auto pb-10 sidebar-container'>
      {activeMenu && (
        <>
